...
export class SessionResolverBase {
  ...
  @graphql.ResolveField(() => [MfaAmrClaim], { name: "mfaAmrClaims" })
  async findMfaAmrClaims(
    @graphql.Parent() parent: Session,
    @graphql.Args() args: MfaAmrClaimFindManyArgs
  ): Promise<MfaAmrClaim[]> {
    const results = await this.service.findMfaAmrClaims(parent.id, args);
  ...
  async getUsers(@graphql.Parent() parent: Session): Promise<User | null> {
