/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { LoyaltyTransactionWhereInput } from "./LoyaltyTransactionWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { LoyaltyTransactionOrderByInput } from "./LoyaltyTransactionOrderByInput";

@ArgsType()
class LoyaltyTransactionFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => LoyaltyTransactionWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => LoyaltyTransactionWhereInput, { nullable: true })
  @Type(() => LoyaltyTransactionWhereInput)
  where?: LoyaltyTransactionWhereInput;

  @ApiProperty({
    required: false,
    type: [LoyaltyTransactionOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [LoyaltyTransactionOrderByInput], { nullable: true })
  @Type(() => LoyaltyTransactionOrderByInput)
  orderBy?: Array<LoyaltyTransactionOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { LoyaltyTransactionFindManyArgs as LoyaltyTransactionFindManyArgs };
