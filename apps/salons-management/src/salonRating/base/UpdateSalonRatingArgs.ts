/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { SalonRatingWhereUniqueInput } from "./SalonRatingWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { SalonRatingUpdateInput } from "./SalonRatingUpdateInput";

@ArgsType()
class UpdateSalonRatingArgs {
  @ApiProperty({
    required: true,
    type: () => SalonRatingWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => SalonRatingWhereUniqueInput)
  @Field(() => SalonRatingWhereUniqueInput, { nullable: false })
  where!: SalonRatingWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => SalonRatingUpdateInput,
  })
  @ValidateNested()
  @Type(() => SalonRatingUpdateInput)
  @Field(() => SalonRatingUpdateInput, { nullable: false })
  data!: SalonRatingUpdateInput;
}

export { UpdateSalonRatingArgs as UpdateSalonRatingArgs };
